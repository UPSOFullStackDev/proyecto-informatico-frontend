...
export const fetchClients = async (user_id) => {
  const clientsUrl = `100.20.92.101:5000/user/${user_id}/clients`;
...
export const fetchProducts = async (user_id) => {
  const productsUrl = `100.20.92.101:5000/user/${user_id}/products`;
  const requestOptions = getRequestOptions();
  try {
...
export const addBill = (selectedClient, billItems, totalBill) => {
  ...
  fetch(`http://100.20.92.101:5000/user/${user_id}/sales`, addBillRequestOptions)
    .then((response) => {
